...
async function start() {
  try {
    ...
    const Matrix = makeWASocket({
      ...
      browser: ["SNOWBIRD-XD", "Chrome", "1.0.0"],
      auth: state,
      getMessage: async (key) => {
        ...
        return { conversation: "SNOWBIRD-XD whatsapp user bot" };
      },
    });
      ...
      if (connection === "open") {
        ...
        if (!hasSentStartMessage) {
          const firstMessage = [
            `◈━━━━━━━━━━━━━━━━◈`,
            `│❒ *${getGreeting()}*`,
            `│❒ Welcome to *SNOWBIRD-XD*! You're now connected.`,
            ``,
            `✨ *Bot Name*: SNOWBIRD-XD`,
          ...
          await Matrix.sendMessage(Matrix.user.id, {
            ...
            contextInfo: {
              externalAdReply: {
                ...
                sourceUrl: `https://github.com/SNOWBIRD0074/SNOWBIRD-XD`,
                mediaType: 1,
...
app.get("/", (req, res) => {
  res.send("SNOWBIRD_XD is running!");
});

const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`SNOWBIRD_MD server running on port ${PORT}`);
});
